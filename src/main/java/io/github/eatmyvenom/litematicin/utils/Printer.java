        ...
        for (int y = fromY; y <= toY; y++) {
            for (int x = fromX; x <= toX; x++) {
                for (int z = fromZ; z <= toZ; z++) {
                    ...
                    if (PRINTER_ALLOW_INVENTORY_OPERATIONS.getBooleanValue() && hasItemInSchematic(world, pos)) {
                        ...
                        if (flipBlocks || requiresMoreAction(stateSchematic, stateClient)) {
                            ...
                            if (!flipBlocks && !stateClient.isAir() && !mc.player.isSneaking() && !isPositionCached(pos, true)) {
                                ...
                                if (stateClient.isOf(Blocks.DIRT)) {
                                    if (stateSchematic.isOf(Blocks.DIRT_PATH) && PRINTER_PRINT_DIRT_VARIANTS.getBooleanValue() && canSwap(mc.player, item -> item.getItem() instanceof ShovelItem)) {
                                    ...
                                    // farmland
                                    else if (stateSchematic.isOf(Blocks.FARMLAND) && PRINTER_PRINT_DIRT_VARIANTS.getBooleanValue() && canSwap(mc.player, item -> item.getItem() instanceof HoeItem)) {
                    ...
                    if (canPickBlock(mc, stateSchematic, pos)) {
                        ...
                        if (blockSchematic instanceof WallMountedBlock || blockSchematic instanceof TorchBlock || blockSchematic instanceof WallSkullBlock
                            ...
                            if (blockSchematic instanceof ButtonBlock || blockSchematic instanceof LeverBlock) {
                                BlockFace BlockFace = stateSchematic.get(WallMountedBlock.FACE);
                                if (BlockFace == BlockFace.FLOOR) {
                                    npos = pos.down();
